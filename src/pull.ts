...
export default async function (options: ReadOptions) {
  ...
  await Promise.all( deleteTargets.map( async article => {
    console.log(`Delete: ${fileRequest.id2filePath(article)}`)
...
const downloadNewerArticle = async (
  ...
  if( localArticle === undefined ) {
    ...
    return {
      ...
      downloaded: true,
      message: `Download: ${request.file.id2filePath(remoteArticle)}${ remoteArticle.draft ? ' (draft)' : '' }`
    }
  }
  ...
  if( localArticle.customUrl !== remoteArticle.customUrl ){
    ...
    messageHead = `${request.file.id2filePath(localArticle)}\n     -> `
  }
  ...
  return {
    article: remoteArticle,
    downloaded: true,
    message: `Update: ${messageHead}${request.file.id2filePath(remoteArticle)}`
...
`Skip to update, because of CONFLICT
 local:  ${request.file.id2filePath(local)}
